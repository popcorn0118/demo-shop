...
class EmailShare extends Base {
		...
		foreach ( $cart_action_list as $action_type ) {
			...
			foreach ( $variant_reward as $campaign_id => $v_reward ) {
				...
				if ( isset( $v_reward['point'] ) && ! empty( $v_reward['point'] ) && $v_reward['point'] > 0 ) {
					...
					Woocommerce::addOneTimeSchedule( 'wlr_scheduled_earn_email_share', [ $schedule_data ], '+1 minute' );
					$status = true;
				}
	...
	function processMessage( $point_rule, $earning ) {
		...
		if ( isset( $point_rule->share_subject ) && ! empty( $point_rule->share_subject ) ) {
			$message['subject'] = Woocommerce::getCleanHtml( __( $point_rule->share_subject, 'wp-loyalty-rules' ) );//phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
		}
		if ( isset( $point_rule->share_body ) && ! empty( $point_rule->share_body ) ) {
			$message['body'] = Woocommerce::getCleanHtml( __( $point_rule->share_body, 'wp-loyalty-rules' ) );//phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
		}
