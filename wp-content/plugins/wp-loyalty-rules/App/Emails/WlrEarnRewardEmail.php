...
class WlrEarnRewardEmail extends \WC_Email {
	...
	public function __construct() {
		...
		add_action( 'wlr_notify_after_add_earn_reward', [ $this, 'trigger' ], 10, 4 );
		parent::__construct();
		$this->description = __( 'This email is sent to the customer when they earn reward', 'wp-loyalty-rules' );
	}

	public function get_default_subject() {
		return __( 'You have earned rewards.', 'wp-loyalty-rules' );
	}
	...
	public function trigger( $email, $reward, $action_type, $data ) {
		$reward_helper = Rewards::getInstance();
		if ( empty( $email ) || ! $reward_helper->is_valid_action( $action_type ) || $action_type == 'point_for_purchase' || ! $this->isEligibleForSentEmail( $action_type, $data ) ) {
			return;
		}
		...
		if ( isset( $data['user_reward_id'] ) && $data['user_reward_id'] > 0 ) {
			...
			if ( ! empty( $user_reward_table->discount_code ) ) {
				$earn_reward_coupon = $user_reward_table->discount_code;
			}
		}
		...
		$this->placeholders = apply_filters( 'wlr_earn_reward_email_short_codes', $this->placeholders, $email, $reward, $action_type, $data );
		...
		$this->placeholders['{wlr_earn_reward_mail_content}'] = $content_html;
		...
		if ( $this->is_enabled() && $this->get_recipient() ) {
			...
			$log_data['note'] = sprintf( __( 'Sending earned %1$s email failed(%2$s)', 'wp-loyalty-rules' ), $reward_label, $campaign_name );
			// translators: 1: label 2: email 3: campaign name
			$log_data['customer_note'] = sprintf( __( 'Sending earned %1$s email failed to %2$s for %3$s campaign', 'wp-loyalty-rules' ), $reward_label, $email, $campaign_name );
			if ( $this->send( $this->get_recipient(), $this->get_subject(), $this->get_content(), $this->get_headers(), $this->get_attachments() ) ) {
				// translators: 1: label 2: campaign name
				$log_data['note'] = sprintf( __( 'Earned %1$s email sent to customer successfully(%2$s)', 'wp-loyalty-rules' ), $reward_label, $campaign_name );
				// translators: 1: label 2: email 3: campaign name
				$log_data['customer_note'] = sprintf( __( 'Earned %1$s email sent to %2$s for %3$s campaign', 'wp-loyalty-rules' ), $reward_label, $email, $campaign_name );
			}
